...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        # Hidden layers
        for i, hdim in enumerate(hidden_dims, start=1):
            self.params[f'W{i}'] = weight_scale * np.random.randn(layer_input_dim, hdim)
            self.params[f'b{i}'] = np.zeros(hdim)

            if self.normalization in ['batchnorm', 'layernorm']:
                self.params[f'gamma{i}'] = np.ones(hdim)
                self.params[f'beta{i}'] = np.zeros(hdim)
    ...
    def loss(self, X, y=None):
        ...
        # final layer (no ReLU / norm / dropout): affine only
        ...
        # L2 regularization loss (only on weights)
        reg_loss = 0.0
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.use_dropout:
                dout_curr = dropout_backward(dout_curr, dropout_caches[i])

            # Depending on normalization setting, use appropriate backward
            if self.normalization is None:
                dout_curr, dW, db = affine_relu_backward(dout_curr, caches[i])
            elif self.normalization == "batchnorm":
                dout_curr, dW, db, dgamma, dbeta = affine_bn_relu_backward(dout_curr, caches[i])
                grads[f'gamma{i}'] = dgamma
                grads[f'beta{i}'] = dbeta
            elif self.normalization == "layernorm":
                dout_curr, dW, db, dgamma, dbeta = affine_ln_relu_backward(dout_curr, caches[i])
                grads[f'gamma{i}'] = dgamma
                grads[f'beta{i}'] = dbeta
