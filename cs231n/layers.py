...
def batchnorm_backward(dout, cache):
    ...
    # backpropagation through scale and shift
    dxhat = dout * gamma                          # (N, D)

    # backpropagation through normalization
    ivar = 1.0 / np.sqrt(var + eps)               # (D,)
    dxmu1 = dxhat * ivar                          # (N, D)

    dvar = np.sum(dxhat * (x - mean) * -0.5 * (var + eps) ** (-1.5), axis=0)  # (D,)
    dmean = np.sum(-dxmu1, axis=0) + dvar * np.sum(-2.0 * (x - mean), axis=0) / N

    dx = dxmu1 + (dvar * 2.0 * (x - mean) / N) + (dmean / N)                  # (N, D)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    ###########################################################################
    # normalize over features (per sample)
...
def layernorm_backward(dout, cache):
    ...
    # per-sample (normalize over features)
    ivar = 1.0 / np.sqrt(var + eps)                      # (N, 1)

    dvar = np.sum(dx_hat * (x - mean) * -0.5 * (var + eps) ** (-1.5),
                  axis=1, keepdims=True)                 # (N, 1)
    dmean = (np.sum(-dx_hat * ivar, axis=1, keepdims=True) +
             dvar * np.mean(-2.0 * (x - mean), axis=1, keepdims=True))  # (N, 1)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        #######################################################################
        mask = (np.random.rand(*x.shape) < p) / p   # keep-prob mask, scaled
        out = x * mask
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def conv_backward_naive(dout, cache):
    ...
    # Pad input and create padded dx
    x_padded = np.pad(
        x,
        ((0, 0), (0, 0), (pad, pad), (pad, pad)),
        mode="constant"
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    # upstream gradient for this position
                    dout_curr = dout[n, f, i, j]

                    # dw: gradient wrt filter f
                    dw[f] += x_padded[n, :, h_start:h_end, w_start:w_end] * dout_curr

                    # dx_padded: gradient wrt input region
                    dx_padded[n, :, h_start:h_end, w_start:w_end] += w[f] * dout_curr

    # Unpad dx to match original x shape
    if pad > 0:
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    # gradient flows only to the max location(s)
                    mask = (window == m)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # reshape to (N*H*W, C) and apply vanilla batchnorm over channels
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # reshape to (N*H*W, C) to match vanilla batchnorm layout
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # reshape into groups: (N, G, C_per_group, H, W)
    x_group = x.reshape(N, G, C // G, H, W)

    # compute mean & var over (C_per_group, H, W) for each group separately
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x_norm_group = x_norm.reshape(N, G, group_size, H, W)
    dx_norm_group = dx_norm.reshape(N, G, group_size, H, W)

    m = group_size * H * W  # elements per group

    # apply groupnorm backward formula (like layernorm per group)
    dx_group = (1.0 / np.sqrt(var + eps)) * (
        dx_norm_group
        - np.mean(dx_norm_group, axis=(2, 3, 4), keepdims=True)
        - x_norm_group * np.mean(dx_norm_group * x_norm_group,
